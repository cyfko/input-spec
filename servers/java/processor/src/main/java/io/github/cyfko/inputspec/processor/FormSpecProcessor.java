...
import io.github.cyfko.inputspec.spi.DefaultFieldContext;
import io.github.cyfko.inputspec.spi.DefaultFormContext;
import io.github.cyfko.inputspec.spi.SpiDispatcher;
 ...
 *   <li>Writes META-INF/input-spec/{formId}.json</li>
 *   <li>Writes META-INF/input-spec/i18n/{formId}.properties (bundle skeleton)</li>
 * </ol>
 *
...
public class FormSpecProcessor extends AbstractProcessor {
    ...
    public synchronized void init(ProcessingEnvironment env) {
        ...
        spi       = new SpiDispatcher(messager);
    }

    @Override
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        for (Element element : roundEnv.getElementsAnnotatedWith(FormSpec.class)) {
            // Accept both classes and interfaces (@Projection DTOs)
            if (element.getKind() != ElementKind.CLASS
                    && element.getKind() != ElementKind.INTERFACE) continue;
            try {
                ...
                messager.printMessage(Diagnostic.Kind.ERROR,
                        "[InputSpec] -  Failed to process @FormSpec on " + element + ": " + e.getMessage()
                                + "\n" + sw,
                        element);
            }
        }
    ...
    private void processTopLevelForm(TypeElement clazz) throws IOException {
        ...
        // Extract i18n entries from the fully-built JSON tree — this captures
        // keys generated by SPI transformers and contributors, which bypass the
        // default bundle-population path.
        extractBundleEntries(root, bundle);

        // Write JSON spec
        String json = MAPPER.writeValueAsString(root);
        writeResource("META-INF/input-spec/" + formId + ".json", json, clazz);

        // Write bundle skeleton
        writeBundleSkeleton(formId, bundle, clazz);

        messager.printMessage(Diagnostic.Kind.NOTE,
                "[InputSpec] -  Generated spec + bundle skeleton for form '" + formId + "'");
    }

    /**
     * Walks the fully-assembled JSON tree and collects every i18n entry into the
     * bundle map. An i18n entry is any JSON object that has both an {@code "i18nKey"}
     * string field and a {@code "default"} string field — matching exactly the
     * structure produced by {@code i18nNode()} in both the processor and the SPI
     * implementations.
     *
     * <p>This is the single mechanism that ensures SPI-generated fields
     * (transformers, contributors) contribute their keys to the bundle skeleton,
     * without requiring the SPI API to expose a separate bundle-entry concept.
     */
    private void extractBundleEntries(JsonNode node, Map<String, String> bundle) {
        if (node == null) return;
        if (node.isObject()) {
            JsonNode keyNode     = node.get("i18nKey");
            JsonNode defaultNode = node.get("default");
            if (keyNode != null && keyNode.isTextual()
                    && defaultNode != null && defaultNode.isTextual()) {
                String key   = keyNode.asText();
                String value = defaultNode.asText();
                // Only add if not already present (default pipeline entries take precedence)
                bundle.putIfAbsent(key, value);
            }
            // Always recurse into object children — the i18n node itself may
            // contain nested structures in future schema versions
            node.fields().forEachRemaining(entry -> extractBundleEntries(entry.getValue(), bundle));
        } else if (node.isArray()) {
            node.forEach(child -> extractBundleEntries(child, bundle));
        }
    }

    // ─── Form node building ──────────────────────────────────────────────────

    private ObjectNode buildFormNode(TypeElement typeEl, FormSpec formSpec,
        ...
        // Collect elements: methods for interfaces, fields for classes
        boolean isInterface = typeEl.getKind() == ElementKind.INTERFACE;
        List<Element> elements = collectElements(typeEl);

        // Locales: default fr+en (could be sourced from a future @FormSpec attribute)
        List<String> locales = List.of("fr", "en");

        // ── SPI + default field building ──────────────────────────────────────
        ArrayNode fieldsArray = root.putArray("fields");
        DefaultFormContext.Builder formCtxBuilder = DefaultFormContext.builder(formId)
                .submitUri(formSpec.submitUri())
                .submitMethod(formSpec.submitMethod())
                .locales(locales)
                .defaultPageSize(20)
                .maxPageSize(100);

        for (Element el : elements) {
            String logicalName = logicalFieldName(el);

            // Build FieldContext and try SPI first
            DefaultFieldContext fieldCtx = new DefaultFieldContext(
                    el, logicalName, elementType(el), formId, locales, typeUtils, elemUtils);

            Optional<SpiDispatcher.TransformResult> transformed = spi.tryTransform(fieldCtx);
            if (transformed.isPresent()) {
                // SPI claimed this element — write generated JSON verbatim
                SpiDispatcher.TransformResult result = transformed.get();
                try {
                    fieldsArray.add(MAPPER.readTree(result.json()));
                } catch (Exception e) {
                    messager.printMessage(Diagnostic.Kind.ERROR,
                            "DIFSP SPI: Invalid JSON returned by FieldTransformer for field '"
                                    + logicalName + "' in form '" + formId + "': " + e.getMessage(), el);
                }
                formCtxBuilder.addTransformedField(result.ref(), result.json(), logicalName);
            } else {
                // Default InputSpec generation — but only if the element carries
                // InputSpec metadata (@FieldMeta or Jakarta constraints).
                // Interface methods with only domain-specific annotations (e.g. @ExposedAs)
                // that no SPI transformer claimed are silently skipped: they are not
                // InputSpec fields and should not appear in the FormSpec.
                boolean hasInputSpecMeta = el.getKind() == ElementKind.FIELD
                        || hasAnnotation(el, "io.github.cyfko.inputspec.FieldMeta")
                        || hasJakartaConstraint(el);
                if (hasInputSpecMeta) {
                    fieldsArray.add(buildFieldNode(el, formId, bundle));
                    formCtxBuilder.addDefaultField(logicalName);
                }
            }
        }

        // ── SPI form contributions (combineWith, pagination, etc.) ────────────
        DefaultFormContext formCtx = formCtxBuilder.build();

        for (String additionalFieldJson : spi.collectAdditionalFields(formCtx)) {
            try {
                fieldsArray.add(MAPPER.readTree(additionalFieldJson));
            } catch (Exception e) {
                messager.printMessage(Diagnostic.Kind.ERROR,
                        "DIFSP SPI: Invalid JSON returned by FormContributor.additionalFields"
                                + " for form '" + formId + "': " + e.getMessage(), typeEl);
            }
        }

        // ── SPI bundle entries — merge into the shared bundle map so that every
        // i18nKey emitted by FormContributor implementations appears in the
        // generated bundle skeleton (META-INF/input-spec/i18n/{formId}.properties).
        bundle.putAll(spi.collectBundleEntries(formCtx));

        // ── crossConstraints: declared @CrossConstraint + SPI contributions ───
        ArrayNode ccArray = root.putArray("crossConstraints");
        for (CrossConstraint cc : collectCrossConstraints(typeEl)) {
            ccArray.add(buildCrossConstraintNode(cc, formId, bundle));
        }
        for (String additionalCcJson : spi.collectAdditionalCrossConstraints(formCtx)) {
            try {
                ccArray.add(MAPPER.readTree(additionalCcJson));
            } catch (Exception e) {
                messager.printMessage(Diagnostic.Kind.ERROR,
                        "DIFSP SPI: Invalid JSON returned by FormContributor.additionalCrossConstraints"
                                + " for form '" + formId + "': " + e.getMessage(), typeEl);
            }
        }
    ...
    private ObjectNode buildFieldNode(Element el, String formId,
                                      Map<String, String> bundle) {
        String fieldName = logicalFieldName(el);
        ...
        AnnotationMirror metaMirror = findAnnotationMirror(el, "io.github.cyfko.inputspec.FieldMeta");
        ...
        FieldTypeInfo typeInfo = resolveElementType(el);
        node.put("dataType", typeInfo.dataType());
        node.put("expectMultipleValues", typeInfo.multiple());

        // required — @NotNull/@NotEmpty/@NotBlank on fields; methods always false
        boolean required = isRequired(el);
        ...
        // OBJECT sub-fields (recursive — only for nested @FormSpec classes, not interface methods)
        if ("OBJECT".equals(typeInfo.dataType()) && typeInfo.nestedClass() != null) {
            ArrayNode subFields = node.putArray("subFields");
            for (Element sub : collectElements(typeInfo.nestedClass())) {
    ...
    private void buildConstraintNodes(Element el, String dataType,
                                      String fieldName, String formId,
    ...
    private void writeBundleSkeleton(String formId, Map<String, String> entries,
        ...
        String path = "META-INF/input-spec/i18n/" + formId + ".properties";
        ...
        try (PrintWriter w = new PrintWriter(new OutputStreamWriter(
            ...
            w.println("#   META-INF/input-spec/i18n/{formId}_{locale}.properties");
    ...
    /**
     * Returns the logical field name for an element:
     * - VariableElement (field): its simple name
     * - ExecutableElement (method): strips get/is prefix
     */
    private String logicalFieldName(Element el) {
    ...
    /** Returns the TypeMirror to analyse for an element (field type or method return type). */
    private TypeMirror elementType(Element el) {
        if (el.getKind() == ElementKind.METHOD)
            return ((ExecutableElement) el).getReturnType();
        return ((VariableElement) el).asType();
    }

    private FieldTypeInfo resolveElementType(Element el) {
        return resolveType(elementType(el));
    }

    private FieldTypeInfo resolveType(TypeMirror type) {
    ...
    private List<Element> collectElements(TypeElement typeEl) {
        boolean isInterface = typeEl.getKind() == ElementKind.INTERFACE;
        return typeEl.getEnclosedElements().stream()
                .filter(e -> {
                    if (isInterface) {
                        // Collect ALL non-default, non-static methods.
                        // SPI transformers may claim methods that carry only domain-specific
                        // annotations (e.g. @ExposedAs) without any @FieldMeta.
                        // Methods that are neither claimed by SPI nor annotated with InputSpec
                        // metadata are silently skipped in the main processing loop, not here.
                        return e.getKind() == ElementKind.METHOD
                                && !e.getModifiers().contains(Modifier.DEFAULT)
                                && !e.getModifiers().contains(Modifier.STATIC);
                    } else {
                        // Fields on classes: exclude static
                        return e.getKind() == ElementKind.FIELD
                                && !e.getModifiers().contains(Modifier.STATIC);
                    }
                })
                .sorted(Comparator.comparingInt(e -> {
                    AnnotationMirror m = findAnnotationMirror(e, "io.github.cyfko.inputspec.FieldMeta");
    ...
    private boolean hasAnnotation(Element e, String fqn) {
        return findAnnotationMirror(e, fqn) != null;
    }

    private boolean hasJakartaConstraint(Element e) {
        return e.getAnnotationMirrors().stream()
                .map(a -> a.getAnnotationType().toString())
                .anyMatch(n -> n.startsWith("jakarta.validation.constraints."));
    }
    ...
    private boolean isRequired(Element el) {
        // Interface methods: never required via Jakarta (no @NotNull on return type)
        if (el.getKind() == ElementKind.METHOD) return false;
